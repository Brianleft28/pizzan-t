...
class ShinyBot:
    ...
    ███████║╚██████╔╝██║  ██║███████╗██║ ╚████║
    ╚══════╝ ╚═════╝ ╚═╝  ╚═╝╚══════╝╚═╝  ╚═══╝
    >> THE HUMANOID HUNTER v5.6 - SENTINEL <<
            """ + "\n")
    ...
    def check_any_name_visible(self, frame):
        """Universal trigger: scans all calibrated slots."""
        h_slots = self.config.get("slots", {})
        for s_id, r in h_slots.items():
            if self.get_slot_data(frame, r)['name']: return True
        r_s = self.config.get("slot_single")
        if r_s and self.get_slot_data(frame, r_s)['name']: return True
        return False
    ...
    def loop(self):
        ...
        while self.running:
            ...
            # --- WATCHDOG: Anti-Stuck (30s) ---
            if time.time() - self.last_action_time > 30:
                self.log("SENTINEL: Anti-stuck triggered! Resetting map state.", "WARN")
                for _ in range(3): self.controller._press('x'); time.sleep(0.5)
                self.last_action_time = time.time()

            # --- 1. SEARCH STATE (Horda vs Patrol) ---
            if not self.is_menu_ready(frame) and not self.check_any_name_visible(frame):
                if mode == "horda":
                    ...
                    time.sleep(5.0) # Wait for animation
                else:
                    ...
                    while (time.time() - st) < burst:
                        ...
                        if self.check_any_name_visible(self.observer.capture_frame()): break
                        time.sleep(0.05)
                    self.controller.key_up(self.ditto_dir)
                
                self.last_action_time = time.time()
                if not self.check_any_name_visible(self.observer.capture_frame()):
                    if mode != "horda" and random.random() > 0.5: self.ditto_dir = 'left' if self.ditto_dir == 'right' else 'right'
                    continue
                else: self.log(">>> BATTLE DETECTED!", "BRAIN")
            ...
            shiny_found = False; is_horde = False; is_target = False; target_name = "Unknown"
            ...
            if res_s['name'] and not shiny_found:
                ...
                if res_s['is_shiny']: shiny_found = True; target_name = res_s['name']
                else:
            ...
            if shiny_found:
                self.log_callback("\n✨✨✨ SHINY DETECTED: " + target_name.upper() + " ✨✨✨")
            ...
            if is_target:
                ...
                while self.running:
                    ...
                    while self.running:
                        ...
                        if not self.is_menu_ready(f_curr) and not self.get_slot_data(f_curr, r_s)['name']:
                            ...
                            self.log(f"HUD hidden. Verifying end for {timeout_s}s...", "BRAIN")
                    ...
                    if mv_s:
                        ...
                        for _ in range(5):
                            ...
                            if pp_curr != 99:
                                if nr: leppas.add((mv_s, True)); break
                            time.sleep(0.3)
                ...
                if not self.is_menu_ready(self.observer.capture_frame()):
                    ...
                    if random.random() > 0.3: self.ditto_dir = 'left' if self.ditto_dir == 'right' else 'right'
                    for _ in range(3): self.controller._press('x'); time.sleep(0.5)
                continue

            # ESCAPE
            self.log("Not target. Escaping...", "INFO")
            self.controller.run_away()
            # Safety escape loop with timeout
            esc_start = time.time()
            while self.check_any_name_visible(self.observer.capture_frame()) and self.running:
