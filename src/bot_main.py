...
class ShinyBot:
  ...
  >> THE HUMANOID HUNTER IS ONLINE <<
    ...
    def is_ui_present(self, frame):
        ...
        if horde_present_count >= 3: return True
    ...
    def is_menu_present(self, frame, debug=False):
        """Verifies if the 'Run' button menu is visible."""
        ...
        return bright_pixels > 80 or any(k in full_text for k in keywords)

    def loop(self):
        mode = self.config.get("mode", "horda")
        ocr_retries = int(self.config.get("ocr_retries", 6))
        ...
        while self.running:
            ...
            # --- 3. STATE: ANALYSIS ---
            self.log_callback("\n  ┌───────────────────────────────────┐")
            self.log_callback("  │         ANALYZING POKÉMON         │")
            self.log_callback("  └───────────────────────────────────┘")
            ...
            for attempt in range(ocr_retries):
                ...
                # Check Single
                if not shiny_found and mode == "single":
                    r_single = self.config.get("slot_single")
                    if r_single:
                ...
                if shiny_found or total_identified > 0: break
                self.log(f"OCR attempt {attempt+1}/{ocr_retries} failed. Retrying...", "OCR")
                time.sleep(1)
